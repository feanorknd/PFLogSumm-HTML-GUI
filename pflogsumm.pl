...
#
# Per-user data
my (%recipUserLocal, $recipUserLocalCnt);
my (%recipUserRemote, $recipUserRemoteCnt);
my (%recipUser, $recipUserCnt);
my (%sendgUser, $sendgUserCnt);
# Per-domain data
my (%recipDomLocal, $recipDomLocalCnt);   # recipient domain data
my (%recipDomRemote, $recipDomRemoteCnt);   # recipient domain data
...
my (
    ...
    $msgsDlvrdRemote, $msgsDlvrdLocal,
...
$msgsDlvrd = $msgsRjctd = $msgsDscrdd = $msgsDlvrdRemote = $msgsDlvrdLocal = 0;

# Init messages received and delivered per hour
my @rcvPerHr = (0) x 24;
my @dlvPerHr = @rcvPerHr;
my @dlvRemotePerHr = @rcvPerHr;
my @dlvLocalPerHr = @rcvPerHr;
...
printf " %6d%s  remote delivered (%d%%)\n", adj_int_units($msgsDlvrdRemote), $msgsDlvrdRemotePct;
printf " %6d%s  local delivered (%d%%)\n", adj_int_units($msgsDlvrdLocal), $msgsDlvrdLocalPct;
...
print_per_hour_summary(\@rcvPerHr, \@dlvPerHr, \@dlvRemotePerHr, \@dlvLocalPerHr, \@dfrPerHr, \@bncPerHr, \@rejPerHr, $dayCnt);
...
print_user_data(\%recipDomRemote, "Remote Domains by message count", $msgCntI, $opts{'u'}, $opts{'q'});
print_user_data(\%recipUserRemote, "Remote Recipients by message count", $msgCntI, $opts{'u'}, $opts{'q'});
print_user_data(\%recipDomLocal, "Local Domains by message count", $msgCntI, $opts{'u'}, $opts{'q'});
print_user_data(\%recipUserLocal, "Local Recipients by message count", $msgCntI, $opts{'u'}, $opts{'q'});
...
sub print_per_hour_summary {
    my ($rcvPerHr, $dlvPerHr, $dlvRemotePerHr, $dlvLocalPerHr, $dfrPerHr, $bncPerHr, $rejPerHr, $dayCnt) = @_;
    ...
    time          received  delivered  remotedlvd  localdlvd  deferred    bounced     rejected
    --------------------------------------------------------------------------------------------
End_Of_Per_Hour_Heading
	...
	foreach $value (@$rcvPerHr[$hour], @$dlvPerHr[$hour], @$dlvRemotePerHr[$hour], @$dlvLocalPerHr[$hour], @$dfrPerHr[$hour], @$bncPerHr[$hour], @$rejPerHr[$hour])
	{
	    my $units = ' ';
